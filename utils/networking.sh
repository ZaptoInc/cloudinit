...
if ! [[ "$RESEAU_CIDR" =~ ^([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)/(3[0-2]|[12]?[0-9])$ ]]; then
  echo "Format invalide. Utilisation : $0 192.168.1.0/24 [RANDOM|NETWORK|MASK|FIRST|LAST|USABLE]"
  exit 1
fi
...
if [[ "$ACTION" == "RANDOM" ]]; then
  ...
  int_to_ip "$MASK"
elif [[ "$ACTION" == "FIRST" ]]; then
  int_to_ip $((NET_INT + 1))
elif [[ "$ACTION" == "LAST" ]]; then
  int_to_ip $((BROADCAST - 1))
elif [[ "$ACTION" == "USABLE" ]]; then
  echo $((BROADCAST - NET_INT - 1))
else
